...
from ypl.random_word_slugs.words import PartsOfSpeech, get_words_by_category
...
DEFAULT_FORMAT_OPTION: FORMAT_OPTION = "kebab"


class Options(TypedDict, total=False):
    parts_of_speech: list[PartsOfSpeech]
    categories: dict
...
def generate_slug(num_of_words: int = DEFAULT_NUMBER_OF_WORDS, options: Options | None = None) -> str:
    """
...
def _get_default_parts_of_speech(num_of_words: int) -> list[PartsOfSpeech]:
    parts_of_speech: list[PartsOfSpeech] = []
...
def get_total_unique_slugs(number_of_words: int = DEFAULT_NUMBER_OF_WORDS, options: Options | None = None) -> int:
    """
    ...
    num_adjectives = len(get_words_by_category("adjective", options.get("categories", {}).get("adjective", [])))
    num_nouns = len(get_words_by_category("noun", options.get("categories", {}).get("noun", [])))
    nums = {"adjective": num_adjectives, "noun": num_nouns}
