...
def add_stripe_source_payment_instrument(connection: Connection) -> None:
    """Add Stripe payment instrument for Stripe payouts."""
    stripe_account_id = os.getenv("STRIPE_ACCOUNT_ID", "0000000000000000000000000000000000000000")
    ...
    with Session(connection) as session:
        ...
        stmt = insert(PaymentInstrument).values(
            ...
            facilitator=PaymentInstrumentFacilitatorEnum.STRIPE,
            identifier_type=PaymentInstrumentIdentifierTypeEnum.PARTNER_IDENTIFIER,
            identifier=stripe_account_id,
        )
...
def remove_stripe_source_payment_instrument(connection: Connection) -> None:
    """Remove the system Stripe payment instrument if it exists."""
    with Session(connection) as session:
        system_user = session.exec(select(User.user_id).where(User.name == "SYSTEM")).first()
