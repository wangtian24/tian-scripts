...
def add_hyperwallet_source_payment_instrument(connection: Connection) -> None:
    """Add hyperwallet payment instrument for hyperwallet payouts."""
    hyperwallet_account_id = os.getenv("HYPERWALLET_ACCOUNT_ID", "0000000000000000000000000000000000000000")
    ...
    with Session(connection) as session:
        ...
        existing = session.exec(
            select(PaymentInstrument.payment_instrument_id).where(
                ...
                PaymentInstrument.facilitator == PaymentInstrumentFacilitatorEnum.HYPERWALLET,
        ...
        stmt = insert(PaymentInstrument).values(
            ...
            facilitator=PaymentInstrumentFacilitatorEnum.HYPERWALLET,
            identifier_type=PaymentInstrumentIdentifierTypeEnum.PARTNER_IDENTIFIER,
            identifier=hyperwallet_account_id,
        )
...
def remove_hyperwallet_source_payment_instrument(connection: Connection) -> None:
    """Remove the system hyperwallet payment instrument if it exists."""
    with Session(connection) as session:
        system_user = session.exec(select(User.user_id).where(User.name == "SYSTEM")).first()
